...
/**
 * <code>SearchCompareDisplayPanel</code> displays a after performing a given search 
 * on it.
 *
 * @author 
 * @version 1.0
 */
public class SearchCompareDisplayPanel extends JPanel implements ActionListener {

    private int[][] edges;              // Array of edges
    private int[][][] treeEdgesDFS; 
    private int[][][] treeEdgesBFS; 
    ...
    public SearchCompareDisplayPanel(int[][] edges, int[][][] treeEdgesDFS,
			     int[][][] treeEdgesBFS) {
	this.edges = edges;
	this.treeEdgesDFS = treeEdgesDFS;
	this.treeEdgesBFS = treeEdgesBFS;
	currentEdge = -1;
	currentStartVertex = 0;
	
	if(treeEdgesDFS[0][0][0] == -1) {
	...
	int largestComp = -1
;
	for(int k = 0; k < treeEdgesDFS.length; k ++) {
	    for(int i = 0; i < treeEdgesDFS[k].length; i ++) {
		if(treeEdgesDFS[k][i][2] > largestComp) {
		    largestComp = treeEdgesDFS[k][i][2];
		}
	    }
	}
	...
	if(currentEdge < treeEdgesDFS[currentStartVertex].length-1) {
	    currentEdge ++;
	    repaint();
	}
    }
	...
	if(hasEulerCircuit) {
	    ...
	    //g.setFont(new Font(g.getFont().getName(), Font.BOLD, 12));
	    g.drawString("Depth First Search", 40, 25);
	    
	    //g.setFont(new Font(g.getFont().getName(), Font.BOLD, 12));
	    g.drawString("Breadth First Search ", 350, 25);
		...
		if(currentEdge < treeEdgesDFS[currentStartVertex].length && 
		   currentEdge >= 0) {
			...
			g.drawLine(getXCoor(treeEdgesDFS[currentStartVertex]
					    [i][0],0),
				   getYCoor(treeEdgesDFS[currentStartVertex]
					    [i][0],0),
				   getXCoor(treeEdgesDFS[currentStartVertex]
					    [i][1],0),
				   getYCoor(treeEdgesDFS[currentStartVertex]
					    [i][1],0));		
		    }
		...
		//	if(currentEdge == -1) {	  
		    ...
		    //	}
		    //	else {
		    if(currentEdge < treeEdgesBFS[currentStartVertex].
		       length &&
		       currentEdge != -1) {
			
			for(int i = 0; i <= currentEdge; i ++) {
			    g2d.setStroke(new BasicStroke(2));	
			    // g.setColor(color[treeEdgesBFS
			    //  [currentStartVertex][i][2]]);
			    g.drawLine(getXCoor(treeEdgesBFS
						[currentStartVertex]
						[i][0],1),
				       getYCoor(treeEdgesBFS
						[currentStartVertex]
						[i][0],1),
				       getXCoor(treeEdgesBFS
						[currentStartVertex]
						[i][1],1),
				       getYCoor(treeEdgesBFS
						[currentStartVertex]
